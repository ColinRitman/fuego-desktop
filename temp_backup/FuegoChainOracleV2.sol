...
/**
 * @title Fuego Chain Oracle V2 - Multi-Layered Proof System
 * @dev Enhanced oracle with txn_extra field proof parsing
 * @notice Solves the "WHO deposited" problem in private Fuego network
 * 
 * PROOF ARCHITECTURE:
 * Layer 1: Undefined Output Anomaly (proves it's a deposit)
 * Layer 2: txn_extra Field (contains recipient + signature)
 * Layer 3: ECDSA Signature Verification (proves control)
 */
contract FuegoChainOracleV2 is AccessControl, ReentrancyGuard, Pausable {
    ...
    ) external onlyRole(ORACLE_OPERATOR_ROLE) nonReentrant whenNotPaused {
        require(txHash != bytes32(0), "Invalid tx hash");
        require(hasUndefinedOutputs, "No undefined output anomaly");
        require(txnExtraData.length >= 181, "txn_extra too short");
    ...
    function _verifyRecipientSignature(
        ...
        try ECDSA.recover(ethSignedMessageHash, signature) returns (address recoveredSigner) {
